...
const FILE_PATH = path.join(process.cwd(), "lib", "data.json");

// Expanded Default Rooms
const DEFAULT_ROOMS = [
  {
    id: "executive-double",
...
function initDB() {
  ...
  // Auto-upgrade database if rooms count is small
  try {
    const existing = JSON.parse(fs.readFileSync(FILE_PATH, "utf8"));
    if (!existing.rooms || existing.rooms.length < 8) {
      existing.rooms = DEFAULT_ROOMS;
