...
export default function RootLayout({ children }) {
  return (
    <html lang="en" className={`${inter.variable} ${playfair.variable} ${alexBrush.variable}`}>
      <body
        className={`${poppins.className} min-h-screen flex flex-col antialiased text-black bg-[#800000] transition-colors duration-300 relative overflow-x-hidden`}
      >
        {/* Animated Background Glow Blobs for Luxury Aesthetic */}
        <div className="absolute top-[5%] left-[-15%] w-[45vw] h-[45vw] rounded-full bg-brand-green bg-glow-blob pointer-events-none z-0"></div>
